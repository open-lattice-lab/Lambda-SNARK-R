...
**Status**: ✅ REVIEWED (M4.2 implementation complete)  
...
This document specifies the design for full R1CS (Rank-1 Constraint System) integration into ΛSNARK-R, enabling proving/verification of arbitrary arithmetic circuits. The design extends the existing polynomial-based prover (M2.2/M2.3) and zero-knowledge system (M3) to support constraint satisfaction proofs.

**Key Design Decisions**:
- **Sparse matrix representation** for A, B, C (typical circuits are <1% dense)
- **Witness-to-polynomial encoding** via coefficient injection (z_i → f(X) coefficient)
- **Constraint polynomial** Q(X) = (Az(X) ⊙ Bz(X) - Cz(X)) / Z_H(X) for batch verification
...
**Security Properties**:
- Soundness: ε ≤ 1/q + Adv_LWE ≈ 2^-44 (preserved from M2.3)
- Zero-Knowledge: Adv_ZK ≤ 2^-128 (inherited from M3)
- Completeness: 100% for valid witnesses satisfying constraints

---

## [Σ] Formal R1CS Definition
...
Where:
  ...
  A, B, C ∈ F_q^{m×n} = constraint matrices (sparse)
  public_inputs = [z_1, ..., z_l] ∈ F_q^l
```

**Witness** z ∈ F_q^n satisfies R1CS if:

```
∀ i ∈ [m]:  (Az)_i · (Bz)_i = (Cz)_i
...
```rust
f(X) = Σ_{i=0}^{n-1} z_i · X^i
```

For R1CS, we extend this to constraint polynomials:

```
A(X, Y) = Σ_{i∈[m], j∈[n]} A_{i,j} · L_i(X) · W_j(Y)
B(X, Y) = Σ_{i∈[m], j∈[n]} B_{i,j} · L_i(X) · W_j(Y)
C(X, Y) = Σ_{i∈[m], j∈[n]} C_{i,j} · L_i(X) · W_j(Y)
...
**Simplification** (for ΛSNARK-R v1): Use **flattened polynomial encoding**:

```
For each constraint i, compute:
  a_i = (Az)_i = Σ_j A_{i,j} · z_j
  b_i = (Bz)_i = Σ_j B_{i,j} · z_j
  c_i = (Cz)_i = Σ_j C_{i,j} · z_j

Encode as polynomials over domain H = {ω^0, ω^1, ..., ω^{m-1}}:
  A_z(X) = Σ_{i∈[m]} a_i · L_i(X)
  B_z(X) = Σ_{i∈[m]} b_i · L_i(X)
  C_z(X) = Σ_{i∈[m]} c_i · L_i(X)

Where L_i(X) = Lagrange polynomial s.t. L_i(ω^j) = δ_{ij}
```
...
Where:
  Z_H(X) = Π_{i∈[m]} (X - ω^i) = X^m - 1  (vanishing polynomial)
```

**Key Property**: Witness satisfies R1CS ⟺ Q(X) is a polynomial (no remainder)

Proof:
```
A_z(ω^i) · B_z(ω^i) = C_z(ω^i)  for all i ∈ [m]
⟺ A_z(X) · B_z(X) - C_z(X) vanishes on H
⟺ Z_H(X) | (A_z(X) · B_z(X) - C_z(X))
⟺ Q(X) ∈ F_q[X]
```

---

## [Γ] Security Gates & Invariants

### 2.1 Soundness Preservation

**Gate**: Soundness error must remain ε ≤ 2^-44

**Analysis**:

Current system (M2.2/M2.3):
```
Prover commits to f(X), verifier checks f(α) at random α
Soundness: Pr[cheat accepted] ≤ deg(f)/q + Adv_LWE
```

R1CS extension:
```
Prover commits to Q(X), verifier checks Q(α)·Z_H(α) = A_z(α)·B_z(α) - C_z(α)
Soundness: Pr[invalid witness passes] ≤ (deg(Q) + m)/q + Adv_LWE
```

**Schwartz-Zippel bound**:
```
If witness is invalid, then Q(X) is not a polynomial (has degree > deg or division fails)
Probability random α satisfies check: ≤ 2m/q  (worst case: 2 polynomials of degree m)

For q = 2^44 + 1, m ≤ 10^6:
  ε ≤ 2·10^6 / 2^44 + Adv_LWE ≈ 2^-24 + 2^-128 ≈ 2^-24
```

**ISSUE**: This exceeds our target ε ≤ 2^-44 for large m!

**Mitigation Options**:
1. **Increase q**: Use q = 2^64 + prime → ε ≈ 2^-44 (but requires larger modulus, performance hit)
2. **Multiple challenges**: Query k random points → ε ≈ (2m/q)^k (but k=2 sufficient: (2·10^6/2^44)^2 ≈ 2^-48)
...
**Gate**: Adv_ZK ≤ 2^-128 (HVZK property from M3)
...
prove_zk_r1cs:
  ...
  5. Derive challenge α via Fiat-Shamir
  6. Open Q'(α)

Simulator (without witness):
  1. Sample random Q'(X) ~ U(F_q^{deg+1})
  2. Commit to Q'(X)
  3. Derive challenge α
  4. Open Q'(α)
```
...
Distributions are identical (one-time pad) → Adv_ZK ≤ Adv_LWE ≈ 2^-128 ✓
```
...
**Proof Sketch**:
```
Given: z ∈ F_q^n satisfies (Az) ⊙ (Bz) = Cz

1. Compute a_i, b_i, c_i for all i ∈ [m]
2. Interpolate A_z(X), B_z(X), C_z(X)
3. Compute Q(X) = (A_z(X)·B_z(X) - C_z(X)) / Z_H(X)
   - By assumption, A_z(ω^i)·B_z(ω^i) = C_z(ω^i) for all i
   - Therefore Z_H(X) divides numerator → Q(X) is polynomial ✓
4. Commit to Q(X), derive α, open Q(α)
5. Verifier checks: Q(α)·Z_H(α) = A_z(α)·B_z(α) - C_z(α)
   - This is polynomial identity, holds for all α ✓

Therefore: Valid witness → Valid proof (completeness = 100%)
```
...
**Strategy**:
- prove_simple: Direct witness→polynomial encoding (unchanged)
...
- **Pros**: Memory O(nnz), typical circuits have nnz ≈ 0.01·m·n
...
**Recommended**: **Option B for construction → Option C for proving**

Rationale:
- HashMap during circuit building (mutations needed)
- Convert to CSR before proving (optimize memory + iteration)
- Typical circuit: m=10^6, n=10^5, nnz=10^7 → 80 MB (CSR) vs 800 GB (dense)
...
- **Cons**: O(m^2) interpolation cost (naïve), requires FFT domain
- **Optimization**: Use NTT (Fast Fourier Transform) → O(m log m)
...
**Option A**: Single challenge α
- Soundness: ε ≈ 2m/q ≈ 2^-24 (insufficient for m=10^6, q=2^44)

**Option B**: Two challenges α, β
```
Check 1: Q(α)·Z_H(α) = A_z(α)·B_z(α) - C_z(α)
Check 2: Q(β)·Z_H(β) = A_z(β)·B_z(β) - C_z(β)

Soundness: ε ≈ (2m/q)^2 ≈ 2^-48 ✓
```

**Option C**: Increase modulus q → 2^64 + prime
- Soundness: ε ≈ 2m/q ≈ 2·10^6/2^64 ≈ 2^-44 ✓
...
- Soundness: ε ≤ (2·10^6/2^44)^2 ≈ 2^-48 < 2^-44 ✓
...
|-----------|-------|------------------|-----|-------|
| Matrices A,B,C | O(m·n) | O(nnz) | O(nnz) | nnz ≈ 0.01·m·n typical |
...
- Dense: 3 matrices × 10^6 × 10^5 × 8 bytes = **2.4 TB** ❌
- Sparse: 3 × 10^7 × (16 bytes key + 8 bytes value) = **720 MB** ✓
- CSR: 3 × (10^7 × 8 + 10^6 × 8) = **264 MB** ✓

### 4.2 Computation Cost

| Operation | Complexity | Notes |
|-----------|------------|-------|
| Matrix construction | O(nnz) | Circuit → sparse matrices |
...
- Matrix ops: 10^7 × 10 ns = **100 ms**
- NTT: 10^6 × log_2(10^6) × 50 ns ≈ **1 sec**
- Polynomial ops: 10^6 × 20 ns = **20 ms**
...
| Evaluate Z_H(α), Z_H(β) | O(1) | Precompute ω^m - 1 |
| Check Q(α)·Z_H(α) = A_z(α)·B_z(α) - C_z(α) | O(1) | Field ops |
| Check Q(β)·Z_H(β) = A_z(β)·B_z(β) - C_z(β) | O(1) | Second challenge |
| LWE opening verification | O(1) | From M2.3 |
| **Total verifier** | **O(l)** | ~**1-2 ms** ✓ |

**Success criteria**: Verifier <1ms per proof → achievable with l ≤ 100 public inputs
...
| Challenge α | 8 bytes | Field element |
| Challenge β | 8 bytes | Second challenge |
| Opening Q(α) | 8 bytes | Field element |
| Opening Q(β) | 8 bytes | Field element |
| Opening metadata | ~100 bytes | SEAL opening proof |
| **Total** | **~1.2 KB** | Meets <50 KB target ✓ |
...
impl SparseMatrix {
    /// Matrix-vector product: result[i] = Σ_j M[i,j] * v[j]
...
pub struct R1CS {
    ...
    /// Evaluation domain size (power of 2, ≥ m for NTT)
    pub domain_size: usize,
    
    /// Root of unity ω for NTT domain
    pub omega: u64,
}

impl R1CS {
    /// Check if witness satisfies constraints: (Az) ⊙ (Bz) = Cz
...
impl CircuitBuilder {
    ...
    /// Add constraint: (Σ a_j·z_j) · (Σ b_j·z_j) = (Σ c_j·z_j)
...
pub fn prove_r1cs(
    ...
    // 5. Derive Fiat-Shamir challenges α, β
...
pub fn verify_r1cs(
    ...
    // 4. Evaluate vanishing polynomial Z_H(α) = α^m - 1
    ...
    // 5. Check constraint equation at α:
    ...
    // 6. Check constraint equation at β (soundness amplification)
...
### Phase 1: Core Data Structures (Week 1) ✅ M4.2 COMPLETE
- [x] Implement `SparseMatrix` (CSR format)
- [x] Implement `R1CS` struct
- [x] Add constraint satisfaction check `is_satisfied()`
- [x] Unit tests for matrix operations (13 tests)

### Phase 2: Circuit Builder (Week 1-2) ✅ M4.3 COMPLETE
- [x] Implement `CircuitBuilder` API
- [x] Add constraint addition and variable allocation
- [x] Convert builder to R1CS (sparse matrices)
- [x] Example circuits (multiplication, addition, boolean, fibonacci) (9 circuits, 11 tests)
...
|----------|-------|------------|
| **Soundness** | ε ≤ (2m/q)^2 ≈ 2^-48 | Two-challenge amplification ✓ |
| **Zero-Knowledge** | Adv_ZK ≤ 2^-128 | Polynomial blinding (M3) ✓ |
| **Completeness** | 100% | Polynomial identity ✓ |
| **Verifier Complexity** | O(l) | <1ms for l ≤ 100 ✓ |
| **Proof Size** | ~1.2 KB | Two openings + metadata ✓ |
| **Prover Time** | O(nnz + m log m) | ~1.2 sec for m=10^6 ✓ |
...
- **Q**: How to choose domain size for NTT? Must be power of 2, ≥ m
...
Check:
  (Az)_1 * (Bz)_1 = 2 * 3 = 6 = (Cz)_1 ✓
  (Az)_2 * (Bz)_2 = 6 * 4 = 24 = (Cz)_2 ✓
```
...
**Status**: ✅ REVIEWED & IMPLEMENTED (M4.2 ✓, M4.3 ✓)  
**Next**: M4.4 R1CS Prover (Lagrange interpolation + quotient polynomial Q(X))
