...
fn test_zk_different_blinding_factors() {
    ...
    let public_inputs = r1cs.public_inputs(&witness);
    assert!(verify_r1cs_zk(&proof1, &public_inputs, &r1cs), "Proof 1 should verify");
    assert!(verify_r1cs_zk(&proof2, &public_inputs, &r1cs), "Proof 2 should verify");
    
    println!("✅ Different blinding factors: r1={}, r2={}", 
...
fn test_zk_vs_non_zk_compatibility() {
    ...
    let proof_non_zk = prove_r1cs(&r1cs, &witness, &ctx, 0x1234)
...
fn test_zk_invalid_witness_rejects() {
    ...
    assert!(result.is_err(), "Invalid witness should fail to produce proof");
...
fn test_zk_blinding_prevents_witness_recovery() {
    ...
    assert_ne!(proof.blinding_factor(), 0, "Blinding factor should be non-zero");
    ...
    println!("✅ Blinding factor r={} prevents witness recovery", proof.blinding_factor());
}
