...
use lambda_snark::{
    prove_r1cs, prove_r1cs_zk, verify_r1cs, verify_r1cs_zk,
    LweContext, Params, Profile, SecurityLevel, R1CS, SparseMatrix,
};
use rand::thread_rng;
...
fn test_non_zk_lagrange_small() {
    ...
    let proof = prove_r1cs(&r1cs, &witness, &ctx, 0x1234)
        .expect("Failed to generate non-ZK proof");
...
fn test_non_zk_lagrange_medium() {
    ...
    let proof = prove_r1cs(&r1cs, &witness, &ctx, 0x5678)
        .expect("Failed to generate non-ZK proof");
...
fn test_non_zk_lagrange_large() {
    ...
    let proof = prove_r1cs(&r1cs, &witness, &ctx, 0x9ABC)
        .expect("Failed to generate non-ZK proof");
...
fn test_non_zk_ntt_small() {
    ...
    let proof = prove_r1cs(&r1cs, &witness, &ctx, 0xDEF0)
        .expect("Failed to generate non-ZK NTT proof");
...
fn test_non_zk_ntt_medium() {
    ...
    let proof = prove_r1cs(&r1cs, &witness, &ctx, 0x1111)
        .expect("Failed to generate non-ZK NTT proof");
...
fn test_non_zk_ntt_large() {
    ...
    let proof = prove_r1cs(&r1cs, &witness, &ctx, 0x2222)
        .expect("Failed to generate non-ZK NTT proof");
...
fn test_zk_lagrange_small() {
    ...
    assert!(proof.blinding_factor != 0, "Blinding factor should be non-zero");
    println!("✅ ZK + Lagrange + m={} verified (r={})", m, proof.blinding_factor);
}
...
fn test_zk_ntt_small() {
    ...
    println!("✅ ZK + NTT + m={} verified (r={})", m, proof.blinding_factor);
}
...
fn test_cross_compatibility_non_zk_to_zk() {
    ...
    let proof_non_zk = prove_r1cs(&r1cs, &witness, &ctx, 0xAAAA)
        .expect("Failed to generate non-ZK proof");
    ...
    // Verify both
    let public_inputs = r1cs.public_inputs(&witness);
    assert!(verify_r1cs(&proof_non_zk, &public_inputs, &r1cs), "Non-ZK proof should verify");
    assert!(verify_r1cs_zk(&proof_zk, &public_inputs, &r1cs), "ZK proof should verify");
...
fn test_different_witness_same_circuit() {
    ...
    assert!(verify_r1cs_zk(&proof1, &public1, &r1cs), "Proof 1 should verify");
    assert!(verify_r1cs_zk(&proof2, &public2, &r1cs), "Proof 2 should verify");
    
    // Proofs should be different (different blinding factors at minimum)
    assert_ne!(proof1.blinding_factor, proof2.blinding_factor, "Blinding factors should differ");
...
fn test_performance_baseline_m200() {
    ...
    let proof = prove_r1cs(&r1cs, &witness, &ctx, 0xEEEE)
        .expect("Failed to generate proof");
    let elapsed = start.elapsed();
    
    let public_inputs = r1cs.public_inputs(&witness);
    assert!(verify_r1cs(&proof, &public_inputs, &r1cs), "Proof should verify");
    ...
    assert!(elapsed.as_millis() < 1000, "Performance regression: took {:?} (expected <1s)", elapsed);
}
...
fn test_performance_zk_overhead() {
    ...
    // Non-ZK time
    let start = Instant::now();
    let _proof_non_zk = prove_r1cs(&r1cs, &witness, &ctx, 0xFFFF)
        .expect("Failed to generate non-ZK proof");
    ...
    println!("✅ Performance ZK overhead: non-ZK={:?}, ZK={:?}, ratio={:.2}×", 
             time_non_zk, time_zk, overhead_ratio);
    
    // ZK should not add more than 10% overhead
    assert!(overhead_ratio < 1.10, "ZK overhead too high: {:.2}× (expected <1.10×)", overhead_ratio);
}
