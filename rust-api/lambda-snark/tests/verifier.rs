...
use lambda_snark::{
    generate_opening, prove_simple, verify_simple, Challenge, Commitment, LweContext, Opening,
    Polynomial, Proof,
};
...
fn test_verify_rejects_forged_opening() {
    ...
    let commitment =
        Commitment::new(&ctx, polynomial.coefficients(), 0x1234).expect("Commitment failed");
...
fn test_verify_deterministic() {
    ...
    for i in 0..10 {
        ...
        assert!(
            valid,
            "Verification should be deterministic (iteration {})",
            i
        );
    }
}
...
fn test_verify_multiple_proofs() {
    ...
    assert!(
        !cross_valid1,
        "Proof 1 with wrong public inputs should fail"
    );
    assert!(
        !cross_valid2,
        "Proof 2 with wrong public inputs should fail"
    );
}
...
fn test_verify_soundness_challenge_binding() {
    ...
    assert!(
        !valid2,
        "Proof should NOT verify with different public inputs"
    );
}
