...
fn load_test_vector(dir: &str) -> (Params, TestVectorInput, TestVectorWitness, TestVectorExpected) {
...
fn test_tv1_multiplication() {
    ...
    let public = input.data.get("public")
    ...
    // Extract witness
    let witness_arr = witness.data.get("witness")
        .and_then(|v| v.as_array())
...
fn test_tv2_plaquette() {
    ...
    let witness_obj = witness.data.get("witness")
    ...
    println!("  θ₁={}, θ₂={}, θ₃={}, θ₄={}", theta1, theta2, theta3, theta4);
    println!("  Sum: {} + {} - {} - {} = {}", theta1, theta2, theta3, theta4, sum);
    assert!(expected.valid);
}
...
fn test_all_vectors_have_consistent_params() {
    ...
    match (&params0.profile, &params1.profile, &params2.profile) {
        (Profile::RingB { n: n0, k: k0, q: q0, sigma: s0 },
         Profile::RingB { n: n1, k: k1, q: q1, sigma: s1 },
         Profile::RingB { n: n2, k: k2, q: q2, sigma: s2 }) => {
            assert_eq!(n0, n1);
            assert_eq!(n1, n2);
            assert_eq!(k0, k1);
            assert_eq!(k1, k2);
