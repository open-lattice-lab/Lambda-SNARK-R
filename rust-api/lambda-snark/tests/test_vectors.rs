...
fn load_test_vector(tv_name: &str) -> (Vec<u64>, R1CS) {
    ...
    let path = workspace_root.join("test-vectors").join(tv_name).join("constraints.json");
    ...
    let witness: Vec<u64> = witness_arr.iter()
    ...
    let r1cs = R1CS::new(a_matrix, b_matrix, c_matrix, TEST_MODULUS)
        .expect("Failed to create R1CS");
...
fn test_tv1_invalid_witness() {
    ...
    let result = r1cs.validate_witness(&witness).expect("FFI call should succeed");
    assert!(!result, "Witness should NOT satisfy constraints after corruption");
}
