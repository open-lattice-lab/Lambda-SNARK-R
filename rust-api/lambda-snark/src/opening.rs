...
mod tests {
    use super::*;
    use crate::{verify_opening_with_context, Commitment, LweContext};
    ...
    fn test_opening_soundness_wrong_polynomial() {
        ...
        assert!(
            !valid,
            "Opening for different polynomial should be rejected by LWE binding check"
        );
    }
    ...
    fn test_opening_out_of_field() {
        ...
        assert!(
            !valid,
            "Opening with out-of-field evaluation should be rejected"
        );
    }
}
